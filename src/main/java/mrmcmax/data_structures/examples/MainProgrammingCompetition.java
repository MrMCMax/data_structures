...
public class MainProgrammingCompetition {
	...
	public static void main(String[] args) {
		...
		algorithm = SCALING_EDMONDS_KARP;
		result = solveProblem();
	...
	public static void setFlowAlgorithm(int alg) {
		algorithm = alg;
	}
	...
	private static String naiveSolution() {
		...
		try {
			...
			for (int i = 0; i < G; i++) {
				groups[i] = new Group(fastReader.nextInt(), fastReader.nextInt(), fastReader.nextInt());
			}

			for (int i = 0; i < A; i++) {
				wifis[i] = new Wifi(fastReader.nextInt(), fastReader.nextInt(), fastReader.nextInt(),
						fastReader.nextInt());
				// connect layer2 -> t
			...
			for (int i = 0; i < W; i++) {
				walls[i] = new Wall(fastReader.nextInt(), fastReader.nextInt(), fastReader.nextInt(),
						fastReader.nextInt());
			}
			...
			for (int i = 0; i < G; i++) {
				for (int j = 0; j < A; j++) {
					...
					if (Utils2D.p2pDistanceSquare(groups[i].pos, wifis[j].pos) <= (wifis[j].radius * wifis[j].radius)) {
						boolean flag = false;
						int k = 0;
						while (k < W && !flag) {
			...
			switch (algorithm) {
			case EDMONDS_KARP:
				M = graph.EdmondsKarp(0, G + A + 1);
				break;
			case SCALING_EDMONDS_KARP:
				M = graph.ScalingAlgorithm(0, G + A + 1);
				break;
			case DINIC:
	...
	public static void timeSolution(String fileName) throws IOException {
		...
		System.out.println("Total time stats using algorithm " + algorithm + ": ");
		printStats(times);
