...
public class FIFOPR_Gl_Gap2 extends FlowAlgorithm {

	public FIFOPR_Gl_Gap2() {
		super("FIFOPR_Gl_Gap2");
	}
	...
	public void initDataStructures() {
		...
		//Create vertices
		Vertex v;
		EraserLinkedList<Vertex> zeroHeight = nonActiveVertices[0];
		for (int i = 0; i < n; i++) {
			v = new Vertex(i);
			v.nonActivePointer = zeroHeight.addAndReturnPointer(v);
			v.activePointer = null;
			vertices.add(v);
		}
		Vertex source = vertices.get(s);
		source.increaseHeightBy(n);
		zeroHeight.remove(source.nonActivePointer);
	...
	protected void initAlgorithm() {
		...
		for (int i = 0; i < adjS.size(); i++) {
			...
			if (e.endVertex != t) {
				...
				if (!w.isActive()) {
					nonActiveVertices[0].remove(w.nonActivePointer);
					w.nonActivePointer = null;
					w.activePointer = activeVertices[0].addAndReturnPointer(w);
				}
	...
	protected void globalRelabel() {
		...
		sink.height = 0;
		nonActiveVertices[0].addAndReturnPointer(sink);
		visited[t] = true;
		visited[s] = true;
		q.reset();
		q.add(sink);
